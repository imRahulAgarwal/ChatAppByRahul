...
const { MAIL_EMAIL, MAIL_PASSWORD, HOST, MAIL_PORT, SECURE, ENV, RESET_PASSWORD_PAGE } = process.env;

const sendForgotPasswordMail = async ({ name, email, token }) => {
    try {
        ...
        if (ENV === "DEVELOPMENT") console.log(getTestMessageUrl(information));
        return;
    } catch (error) {
